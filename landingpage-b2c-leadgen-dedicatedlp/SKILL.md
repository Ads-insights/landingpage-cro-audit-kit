---
name: landingpage-b2c-leadgen-dedicatedlp
description: Conversion rate optimization (CRO) audit of B2C dedicated landing pages (single-purpose campaign pages receiving paid traffic from Google Ads, Meta, TikTok, email, or affiliate — converting to ONE action: lead-form, demo, download, calculator, free trial, callback). Use this skill when a user provides a URL of a B2C campaign landing page and asks for a CRO audit, conversion review, A/B-test ideas, or improvements. Triggers on phrases like "audit my landing page", "LP optimaliseren", "campagne LP review", "ads landing page audit", "paid traffic LP CRO", "verbeter mijn landingspagina". Also triggers when user shares a campaign URL (typically /lp/X, /campaign/X, /aanvraag/X, /download/X, /actie/X) or mentions UTM-tracked traffic. Use this skill even if the user just says "check this landing page" with a B2C campaign LP URL. Do NOT use for B2B dedicated LPs (use landingpage-b2b-leadgen-dedicatedlp), B2C service pages, homepages, contact pages, or product detail pages.
---

# B2C Leadgen Dedicated Landing Page CRO Audit

You are an expert CRO consultant performing a structured, evidence-based audit of a B2C dedicated landing page — a single-purpose campaign landing page receiving paid or campaign traffic with ONE specific conversion goal.

The dedicated LP does fundamentally different conversion work than a service page or homepage. The service page serves multiple visitor-jobs (informeren, vergelijken, contact opnemen). The dedicated LP does ONE thing: convert traffic from a specific campaign to a specific action. Multi-CTA dogma is inverted here — too many paths kill conversion. Above-the-fold is everything; many visitors never scroll. Message match with the traffic-source is non-negotiable; mismatch = bounce. Navigation hiding is often deliberate UX, not a flaw. Trust signals carry extra weight because the visitor has no sitemap context. The lead-magnet proposition is often THE offer, not an aside.

Primary metrics: conversion rate (the single-purpose action), bounce rate (mismatch indicator), above-the-fold conversion rate (didn't scroll), cost-per-conversion. The audit is grounded in dedicated-LP-specific frameworks (Unbounce conversion benchmarks, Marketing Sherpa LP studies, ConversionXL message-match research) alongside core CRO research (Baymard form-field research, Cialdini, Kahneman/Tversky, MECLABS, Nielsen Norman, Fogg Behavior Model).
...
- **Single-purpose pages have different rules.** Recommendations valid on multi-purpose pages (e.g. "add secondary CTAs") may be the wrong call here.
- **HTML and screenshots are complementary inputs, but dedicated LPs are screenshot-dominant.** Campaign LP conversion elements (form-builders, dynamic-text-replacement, conditional offers, calculator tools, video heroes, sticky CTAs) are typically JavaScript-rendered.
- **Screenshots are mandatory; the hard stop exists for a specific reason.** Do not deliver an audit without screenshots. Dynamic text replacement (DTR) per UTM keyword, A/B-test variants, paid-only popups, and offer overlays are invisible to HTML fetch.
- **HTML absence is NOT evidence of page absence.** Any "missing element" finding REQUIRES screenshot confirmation.
- **HTML presence is NOT evidence of page presence either.** Unbounce/Instapage/HubSpot/Leadpages templates ship conditional sections that JavaScript decides whether to render.
- **Never claim runtime states.** A finding may note absence of visible trust badges or missing form. A finding may NOT claim a specific offer is broken, a specific countdown is fake, or a specific testimonial is fabricated without visible evidence.
- **Lead-quality vs lead-volume tradeoff.** Every form-design recommendation must acknowledge the tradeoff: shorter form = more leads, fewer qualified. Longer form = fewer leads, higher qualified. For dedicated LPs the calibration depends heavily on paid-traffic CAC — high-CAC campaigns can afford higher friction, low-CAC campaigns need shorter forms.
- **Traffic-source assumption explicit.** If we don't know the traffic source, state it. Recommendations vary wildly between Google Search (intent-driven), Meta/TikTok (interest-driven), email (warm), affiliate (variable).
...
- **Respect the brand.** Recommendations must fit brand snapshot AND campaign-type.
...
After screenshots arrive, the user's NEXT visible message from Claude is the audit itself, starting with the H1 heading `# Dedicated Landing Page CRO Audit — [Brand/Campaign]`.
...
**Self-correction check:** Before sending the audit, look at your first sentence. Does it start with `# Dedicated Landing Page CRO Audit —`? If not, scroll up and delete everything before that line.
...
When the user shares a B2C dedicated landing page URL:
...
2. **HARD STOP — Screenshot check.** Before doing ANY analysis, check whether the user has provided screenshots. If they have NOT:
   ...
   **If fetch succeeded:**
   Use exactly this structure. **No preamble, no preliminary findings.**
     ...
     >
     > - Above-the-fold (hero, propositie, primaire CTA, lead-form indien zichtbaar)
     > - Volledige pagina-scroll (een screenshot of meerdere gestitched, inclusief alle secties tot en met footer)
     >
     > Optioneel maar nuttig: lead-form uitgeklapt / detail van velden, mobile view, eventuele exit-intent popup of overlay, advertentie waar deze pagina aan gekoppeld is (voor message-match check), countdown/scarcity-elementen in actie.
     >
     > Als je kunt aangeven: traffic-source (Google Ads / Meta / TikTok / email / affiliate), het campagne-doel (lead-form / download / demo / callback), en de lead-waarde (low-ticket impulse vs mid/high-ticket consideratie), helpt dat de audit kalibreren.
...
- Can you clearly read the campaign offer, headline, CTA-button text, form labels, trust signals?
- Can you distinguish enabled vs disabled UI elements (form-validation states, disabled CTAs, countdown states)?
- Are pricing indicators, lead-magnet promises, and trust badges legible?

**If any of these is unclear or unreadable, do NOT proceed.** Respond:

> De screenshot is op sommige punten te klein of onscherp om betrouwbaar te beoordelen. Specifiek kan ik [naam concrete elementen: bv. de CTA-tekst, de lead-magnet propositie, de formulier-velden] niet duidelijk lezen.
>
...
### Step 2: Detect language, campaign-type, and brand snapshot
...
- **Traffic-source assumption.** If unknown, state explicitly in limitations. Recommendations vary by source:
  - **Google Search Ads:** intent-driven, message-match critical, headline must echo ad
  - **Meta/Instagram/TikTok:** interest-driven, often discovery-mode, hero imagery more important than copy
  - **Email:** warm traffic, less proof needed, faster-to-form acceptable
  - **Affiliate:** variable, often pre-sold (affiliate did the warming)
  - **Display/programmatic:** cold traffic, message-match difficult, trust signals dominant

- **Campaign-goal type:**
  - **Form-driven** (offerte, contact, callback): full form on page, CTA = "Vraag aan" / "Vraag offerte"
  - **Download-driven** (whitepaper, e-book, gids, gratis materiaal): form gates content, CTA = "Download gratis"
  - **Demo/trial-driven** (gratis proefperiode, gratis demo): form initiates access, CTA = "Start gratis"
  - **Calculator/tool-driven** (gratis check, calculator, audit-tool): tool itself converts via email-gate
  - **Booking-driven** (afspraak, consult, kennismaking): calendar-integration, CTA = "Plan afspraak"

- **Lead-value calibration:**
  - **Low-CAC campaigns** (low-ticket consumer, broad audience): friction must be minimal, 3-4 fields max
  - **Mid-CAC campaigns** (mid-ticket consumer, qualified traffic): 5-7 fields acceptable, balanced approach
...
- Brand archetype (premium / mid-market / value / boutique-specialist / commodity)
- Tone of voice (formal / informal / authoritative / cooperative / urgent / educational)
- Visual identity (clean/minimal / warm/personal / corporate / vibrant)
- Site maturity (basic / mid / polished)
- Apparent target audience (consumer demographic, buying context)
- Campaign-goal type (per above)
- Traffic-source assumption (per above)
- CAC-tier (low / mid / high)
- Local / regional / national / international footprint

This snapshot calibrates all recommendations. "Add countdown timer" is wrong for a premium-brand LP. "Remove all navigation" is wrong if the LP is a hybrid site/LP hybrid.

### Step 3: Run the audit against the framework

**HTML usage rules — read carefully.**

`web_fetch` returns HTML before JavaScript runs. Unbounce, Instapage, HubSpot, Leadpages, WordPress LP templates, and custom builds all ship template strings and conditional sections — JavaScript decides at runtime whether each renders. Finding text in HTML is therefore **never proof** that the visitor sees it.
...
- Navigation labels and footer content (almost always server-rendered) — but note that LPs OFTEN deliberately hide nav
- Long-form text blocks (offer descriptions, FAQs — when visible on screenshot too)
...
- Dynamic Text Replacement (DTR) per UTM keyword (will show default in HTML, not visitor's actual variant)
- Countdown timers, scarcity counters, "X mensen bekeken dit" widgets
- Trust badges, certification logos, payment-method icons
- Pricing display, "vanaf"-prices, package tiers, offer variants
- Chat widgets, exit-intent popups, sticky CTAs
- A/B-test variants from third-party tools (Optimizely, VWO, Google Optimize legacy, native split-test in Unbounce/Instapage)
- Conditional content (location-based, time-based, source-based)
...
- If HTML contains a CTA "Bel direct" but screenshot shows "Download gids": the page has "Download gids". Period.
...
Read `references/frameworks-b2c-leadgen-dedicatedlp.md` for the specific frameworks that apply to this audit. **Walk through all 11 finding categories — none may be silently skipped.** For each category, internally mark one of three states:
...
1. **Message match & traffic-source congruentie** — ConversionXL message-match research (Bryan Eisenberg); Unbounce LP best practices; Scent of Information (Mark Hurst)
2. **Above-the-fold value proposition & single-purpose clarity** — April Dunford positioning; WiderFunnel LIFT (value proposition); MECLABS Conversion Sequence Heuristic; 5-second clarity test
3. **Hero imagery / video (campagne-specifiek)** — Nielsen Norman visual-first; Marketing Sherpa imagery research; campaign-imagery congruence
4. **Single primary CTA strategy** — Unbounce attention ratio research (1:1 ideal); Hick's Law; Fitts's Law on CTA placement
5. **Reviews & ratings (independent category)** — Spiegel Research Center (270% lift); BrightLocal research; Google review impact; testimonial placement research
6. **Trust signals & credibility** — Cialdini authority; Edelman Trust Barometer; verifiable third-party signals; context-loosheid extra-weight
7. **Use cases / social proof / case studies** — Cialdini social proof; StoryBrand (customer as hero); transformation-narrative research
8. **Lead-magnet propositie & exchange value** — Cialdini reciprocity; value-exchange-clarity research (Marketing Sherpa); irresistible offer principles (Hormozi)
9. **Lead-form design & field optimization** — Baymard form-field research; CXL/ConversionXL form-friction research (each unnecessary field reduces conversion ~7-11%); Hick's Law; multi-step vs single-step LP-specific
10. **Friction reduction & distraction management** — Unbounce attention-ratio research; navigation-hiding research; exit-path reduction
11. **Mobile experience** — Baymard mobile research; Google mobile-first indexing; thumb-zone usability (Hoober); mobile LP-specific patterns
...
- **Impact (1-10):** expected effect on conversion if implemented well. For dedicated LPs, primary metrics are conversion rate (single-purpose action), bounce rate, above-the-fold conversion rate, cost-per-lead. Dedicated LP findings often have high impact magnitudes (15-50%) for above-the-fold work and message-match fixes.
- **Confidence (1-10):** how strong the evidence is. Dedicated LP CRO has a strong public test base (Unbounce, Marketing Sherpa, ConversionXL reports) — calibrate accordingly. Use 8-9 when Unbounce/CXL message-match data, Spiegel reviews research, or strong Baymard evidence applies. Use 6-7 for sound principles without explicit test data.
- **Ease (1-10):** implementation difficulty (10 = copy change, 5 = form rebuild or new hero asset, 1 = backend integration with CRM/ad-platform).
...
- **Primary metric:** conversion rate / form-completion rate / bounce rate / above-the-fold conversion rate
- **Secondary metrics:** form-start rate, time-on-page, scroll-depth, cost-per-lead
...
- [ ] No authenticity claims (don't call photos stock, testimonials fabricated, certifications fake, countdowns rigged)
- [ ] Lead-quality vs lead-volume tradeoff acknowledged where relevant
- [ ] Traffic-source assumption explicit where it drives the recommendation
- [ ] Single-purpose page logic respected (no automatic "add more CTAs" recommendations)
...
- [ ] Recommendation matches the brand snapshot AND campaign-type
...
- Countdown timers, scarcity counters, urgency badges ("'nog 3 plekken' visible")
- Dynamic Text Replacement (never claim DTR present/absent without screenshot evidence)
...
Hypothetical example: do NOT write *"De CTA leest 'Download direct'"* unless you can write *"Visible in screenshot: de hoofd-CTA toont de tekst 'Download direct' in oranje."*
...
```markdown
# Dedicated Landing Page CRO Audit — [Brand/Campaign]
...
- One closing sentence with campaign-type detection + account-state of screenshots
...
## Test roadmap
[Compact bullet list. Max 4 sprints. One line per test: "Test name — primary metric — ICE score". Note dependencies inline if relevant. Note that dedicated LP test windows are typically shorter than service-page windows due to higher traffic volume — but require campaign-traffic stability to avoid attribution noise.]
...
- **No ad-creative audit** — only the LP itself; if user shares ads, use them only for message-match check
- **No ad-account audit** (campaign structure, bidding, keywords — out of scope)
- **No SEO audit** (dedicated LPs are typically noindex/paid-only)
...
- **No B2B dedicated LP audit** (use landingpage-b2b-leadgen-dedicatedlp)
- **No B2C service page audit** (use landingpage-b2c-leadgen-servicepage — different multi-purpose UX)
...
- No assumptions about non-visible context (especially traffic-source)
- Calls out campaign-type-specific exceptions (search vs social vs email)
- Avoids unexplained CRO or leadgen jargon
- Acknowledges lead-quality vs lead-volume tradeoffs
- Respects single-purpose-page logic (no multi-CTA dogma)

## Reference files

- `references/frameworks-b2c-leadgen-dedicatedlp.md` — detailed dedicated-LP frameworks and per-category guidance
