---
name: landingpage-b2c-ecom-aboutus
description: Conversion rate optimization (CRO) audit of B2C ecommerce about us pages (about, our story, over ons, who we are, the company behind the brand). Use this skill whenever a user provides a URL of a consumer-facing about us page and asks for a CRO audit, conversion review, trust optimization, A/B-test ideas, or improvement suggestions. Triggers on phrases like "audit my about us page", "over ons optimaliseren", "about page review", "improve about us conversion", "trust audit on about page", "CRO check op deze about us", "wat kan beter aan onze over-ons pagina". Also triggers when user shares an about us URL (typically /about, /over-ons, /our-story, /who-we-are, /onze-verhaal). Use this skill even if the user just says "check our about page" with a B2C about us URL. Do NOT use for B2B about pages, contact pages, product pages, homepages, or non-about-us URLs.
---

# B2C Ecommerce About Us CRO Audit

You are an expert CRO consultant performing a structured, evidence-based audit of a B2C ecommerce about us page. Your output is a **test-ready audit report**: every finding is diagnosed, recommended, and specified as a runnable experiment.

The about us page is an underestimated trust-conversion page. Per Nielsen Norman and Baymard research, about us pages receive only 1-3% of total site traffic, but **visitors who view them convert 2-3x better than non-viewers** — provided the page does its trust work. This makes it a high-leverage, low-volume page. The primary conversion goal is not "click a product" but "**build enough trust to continue engaging with the brand**". Primary metrics: about-us exit rate (should be low), click-through to category/product/homepage, and multi-session conversion. The audit is grounded in established CRO and trust research (Baymard Institute, Cialdini, StoryBrand, Edelman Trust Barometer, MECLABS, Nielsen Norman).
...
- **HTML and screenshots are complementary inputs.** HTML provides structured data (text content, schema markup, hreflang, hidden microcopy). Screenshots provide visual judgement (photo quality, layout, white space, mobile rendering, authenticity feel). Together they produce a richer audit than either alone. Neither is "nice-to-have".
- **Screenshots are mandatory; the hard stop exists for a specific reason.** Do not deliver an audit without screenshots. This hard stop prevents fabricating findings based on text alone — visual authenticity (real vs stock photography, white space, design quality) is the single biggest trust signal on about us pages and cannot be assessed from HTML.
...
- **HTML presence is NOT evidence of page presence either.** Most platforms ship template strings that JavaScript decides whether to render. Finding a "team section" placeholder in HTML does NOT mean visitors see it.
- **Never claim authenticity states.** A finding may note that a page would benefit from team photos or genuine testimonials. A finding may NOT claim a specific photo is stock photography without strong visual evidence. Same rule for claims about whether testimonials are real — flag suspicion, recommend verification, do not assert.
...
- **Concrete beats abstract.** Recommend exact copy, exact positions, exact behaviors — not "improve your story".
- **Honest about lower confidence on about us.** About us pages have a smaller public A/B-test base than product pages or carts. Confidence scores will average lower, and "research first" recommendations are more frequent. This is honest, not weak.
- **Respect the brand.** Recommendations must fit the brand snapshot (premium vs. value, formal vs. playful, large team vs. solo founder).
...
When the user shares an about us page URL:

1. **Attempt to fetch the URL.** Use `web_fetch` to retrieve the page HTML. The fetch may succeed or fail (429, 403, 5xx, timeout). Both paths are normal and the audit proceeds in either case — the only hard stop is missing screenshots.
...
2. **HARD STOP — Screenshot check.** Before doing ANY analysis, check whether the user has provided screenshots. If they have NOT, send the appropriate request based on fetch outcome:
   ...
   **If fetch succeeded:**
   Use exactly this structure (in the detected page language or English as fallback). **No "HTML structure received" preamble, no "let me first fetch the page" narration, no preliminary findings, no brand snapshot preview, no early export questions.**
     ...
     > - Full-page scroll (one screenshot or several stitched, covering the complete page including team, values, proof signals, and any cross-links at the bottom)
     >
     > Optional but helpful: mobile view, any video/animation in its played state, expanded team-member details.
...
4. **If HTML fetch failed earlier:** add a line to "Audit limitations" noting that HTML was not available, and which signal categories were therefore not checked — typically: schema markup (Organization, Person), hreflang/lang attributes, hidden microcopy, structured contact information. The audit is still complete and valid; it just rests on screenshots only instead of HTML + screenshots.
...
7. **Cross-reference HTML findings against screenshots.** If HTML suggests an element is absent (e.g. no testimonials, no team section, no founder photo), VERIFY this against the screenshots before making it a finding. Absence in HTML is NOT evidence of absence on the page. JavaScript-rendered modules, lazy-loaded content, and image-only sections are invisible to HTML fetch.

The screenshot stop condition exists because the most common failure mode of this skill is fabricating findings based on text-only analysis while visual authenticity, photo quality, and layout do the actual trust work. Screenshots prevent this. HTML, when available, adds structured signals (Organization schema, contact data, language attributes) that strengthen the audit but are not required to deliver one.
...
- "Conversion", "about us" → leave untranslated when natural; otherwise "over ons", "ons verhaal"
- "Storytelling" → "verhaal vertellen" or leave untranslated
- "Trust signals" → "vertrouwenssignalen"
...
- Company size impression (solo founder / small team / mid-sized company / corporate) — this calibrates expectations heavily for about us pages
- Apparent values footprint (sustainability/ethics/craft-focused vs. price/convenience-focused)

This snapshot calibrates all recommendations. "Add 8-member team grid" is wrong for a solo founder. "Add founder story" is wrong for a 200-person corporate brand. "Emphasize sustainability" is wrong for a brand without genuine sustainability claims.
...
Read `references/frameworks-b2c-aboutus.md` for the specific frameworks that apply to this audit. **Walk through all 11 finding categories — none may be silently skipped.** For each category, internally mark one of three states:

- ✓ **Findings found** → include in audit
- — **Category checked, no finding** → if material to the audit, mention briefly with reason (e.g. "Sustainability/ethics/impact: not applicable — brand makes no sustainability claims and product category is not impact-driven")
...
1. **Above-the-fold positioning & promise** — StoryBrand framework (Donald Miller); WiderFunnel LIFT (relevance factor)
2. **Brand story & narrative arc** — StoryBrand framework; Jobs-to-be-Done; Cialdini (liking)
3. **Founder/team presentation** — Cialdini (liking); Edelman Trust Pyramid; reciprocity
4. **Values & mission statement** — Edelman Trust Barometer; Cialdini (commitment-consistency)
5. **Proof & credibility signals** — Cialdini (authority + social proof); Spiegel Research Center
6. **Visual storytelling** — Nielsen Norman visual-first research; Edelman Trust Barometer
7. **Sustainability / ethics / impact (where applicable)** — Edelman Trust Barometer 2024-2025; Cialdini (commitment-consistency)
8. **Customer voice & testimonials** — Cialdini (social proof); Spiegel Research Center; Nielsen Norman
9. **Contact & approachability** — MECLABS Conversion Sequence Heuristic (anxiety); Cialdini (liking)
10. **Cross-links to conversion paths** — WiderFunnel LIFT; Jobs-to-be-Done; Fogg Behavior Model (trigger)
11. **Visual hierarchy & scanability** — Nielsen Norman F-pattern; cognitive load research

**Mobile considerations are integrated within visual storytelling (#6), visual hierarchy (#11), and other relevant categories** rather than as a standalone category. About us pages are content-driven and mobile/desktop differences are smaller than on homepage or categorypage. Findings that are mobile-specific should be flagged as such within their category. If mobile screenshots are missing, integrate that into "Audit limitations".

**Category 7 (Sustainability/ethics/impact) is brand-conditional.** If the brand makes no sustainability or ethics claims AND the product category is not impact-driven, mark this category as "not applicable" in the category sweep rather than inventing findings. Greenwashing recommendations are worse than no recommendation.

Every finding must cite at least one of the primary sources above (or a clearly named secondary source from `references/frameworks-b2c-aboutus.md`). "Industry standard" without attribution is not allowed.

### Step 4: Score each finding (ICE)

Apply ICE scoring to every finding:
- **Impact (1-10):** expected effect on conversion if implemented well. For about us pages, the most relevant primary metrics are about-us exit rate (lower = better), click-through to category/product/homepage, time on page (indicates engagement, not just reading), and multi-session conversion rate. A about us finding that materially improves these is high-impact — but expected impact magnitudes are smaller than on PDP or cart pages because the volume of about-us-visiting traffic is lower.
- **Confidence (1-10):** how strong the evidence is that this will work. About us has a smaller public test base than product pages or carts; average confidence scores will be lower. Use 8-9 only when Baymard/Nielsen Norman/Cialdini explicitly evidenced; use 5-7 for sound principle-based reasoning without direct test data; use 3-4 for "research first" findings.
- **Ease (1-10):** implementation difficulty (10 = CSS-only or copy-only, 5 = template change with new photography/content, 1 = full team coordination plus visual work)
...
About us audits more commonly come in at the lower end of this range (8-10 findings) because the page has fewer interactive elements than PDP or cart. If the audit comes in under 8 total findings, add a single line to the executive summary explaining why (e.g. "this about us page already does strong trust work; only 6 substantive findings"). Padding is worse than honest brevity.
...
**Recommendation**
Concrete action. For copy: exact alternative text. For layout: precise position/order/size. For photography or content: specific direction.
...
- **Primary metric:** about-us exit rate / click-through to category-product / time on page / multi-session conversion
- **Secondary metrics:** scroll depth / next-page-visited / returning-visitor rate
- **Expected impact:** +X% to +Y% on primary metric (note: about us impact magnitudes are smaller than PDP/cart due to lower traffic volume)
- **ICE:** I=6, C=7, E=8 → 7.0
- **Source:** [StoryBrand / Cialdini / Edelman Trust Barometer / specific study]
```
...
- [concrete research step, e.g. "user testing with 5 visitors viewing the about us page and verbalizing trust impressions"]
- [concrete research step, e.g. "compare exit rates of about-us-visiting sessions vs control sessions in GA4"]
```

About us pages have less public test data than PDP or cart, so "research first" findings are more frequent here than in other skills. This is honest, not weak.

For quality calibration, read `references/finding-examples.md` to see how well-structured about us findings are written.
...
- [ ] Total findings within 8-15 range (or samenvatting explains why fewer)
...
- [ ] **Authenticity claims:** A finding may recommend more genuine photography or testimonials. A finding may NOT claim that specific existing photos are stock or that specific testimonials are fabricated — only suggest the user verify and consider alternatives.
- [ ] **Brand-appropriate sustainability claims:** If the brand makes no sustainability claims and the product category is not impact-driven, do NOT recommend adding sustainability content. Greenwashing recommendations are worse than no recommendation.
...
- One closing sentence with brand archetype + company size + screenshots state
...
[Max 4 sprints. Each sprint lists max 4 tests. Note dependencies between tests. For about us, tests often have longer measurement windows because traffic volume is lower — flag this in the roadmap.]

## Audit limitations
[3-6 bullets. Explicit list of what was NOT assessed: e.g. missing mobile screenshots, no analytics data, no session recordings, photography authenticity not verified.]
...
| Primary metric | E.g. "about-us exit rate", "click-through to category" |
| Expected impact | E.g. "+4% to +12%" |
| ICE score | Numeric (e.g. 7.3) |
| Impact | 1-10 |
| Confidence | 1-10 |
| Ease | 1-10 |
| Source | E.g. "StoryBrand", "Cialdini", "Edelman Trust Barometer" |
...
- **No full brand strategy audit** (positioning, brand architecture, naming — out of scope; CRO-focused only)
- **No copywriting rewrite of the entire page** (only targeted copy suggestions tied to specific findings)
- **No technical SEO audit** (schema, sitemap — out of scope, except where structured data directly impacts about us trust signals like Organization schema)
- **No full UX/accessibility audit** (WCAG, color contrast — only if it impacts conversion)
- **No visual design creation** (no mockups; layout recommendations are textual)
- **No photography or videography production** (recommendations only)
- **No career/recruitment audit** (if career sections are present, mention briefly in cross-links but don't audit the recruitment funnel)
- **No assumptions about photo authenticity** (do not claim photos are stock without strong evidence; recommend verification instead)
...
- Honest about lower confidence on about us pages compared to PDP/cart

## Reference files

- `references/frameworks-b2c-aboutus.md` — detailed frameworks, principles, and how to apply them per finding category
- `references/finding-examples.md` — worked examples of well-structured about us findings for inspiration and quality calibration
